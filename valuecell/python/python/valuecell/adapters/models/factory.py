...
from typing import Any, Dict, Optional  # 修复：移除重复的Any
import requests
...
class AlphaVantageProvider(ModelProvider):
    ...
    def create_model(self, model_id: Optional[str] = None, **kwargs):
        """创建Alpha Vantage数据客户端"""
        # Alpha Vantage不是LLM模型，返回一个数据客户端实例
        return AlphaVantageClient(self.config.api_key, self.base_url)
    
    def is_available(self) -> bool:
        """检查Alpha Vantage API密钥是否可用"""
        return bool(self.config.api_key)


class TwelveDataProvider(ModelProvider):
    """Twelve Data financial market data provider"""
    ...
    def create_model(self, model_id: Optional[str] = None, **kwargs):
        """创建Twelve Data数据客户端"""
        return TwelveDataClient(self.config.api_key, self.base_url)
    
    def is_available(self) -> bool:
        """检查Twelve Data API密钥是否可用"""
        return bool(self.config.api_key)
...
class AlphaVantageClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://www.alphavantage.co/query"):
        ...
        self.session.headers.update({'User-Agent': 'ValueCell-Forex-Agent/1.0'})
    
    def get_forex_rates(self, from_currency: str, to_currency: str) -> Dict:
        """获取外汇汇率"""
        params = {
    ...
    def get_news_sentiment(self, tickers: str = "FOREX", topics: str = "economy_monetary") -> Dict:
        """获取新闻情绪数据"""
        params = {
    ...
    def get_economic_indicators(self, function: str = "CPI", interval: str = "monthly") -> Dict:
        """获取经济指标数据"""
        params = {
            'function': function,
            'apikey': self.api_key,
            'interval': interval if function in ['CPI', 'INFLATION'] else None
        }
        ...
        try:
            ...
            if 'data' in data or function in ['CURRENCY_EXCHANGE_RATE']:
                return data
            elif 'Error Message' in data:
                return {"error": data['Error Message']}
            elif 'Note' in data:
    ...
    def health_check(self) -> Dict:
        ...
        try:
            ...
            status = "operational" if response.status_code == 200 and 'Realtime Currency Exchange Rate' in data else "degraded"
            
            return {
                "status": status,
                "api_calls_today": "unknown",  # Alpha Vantage不提供使用量信息
...
class TwelveDataClient:
    ...
    def get_forex_rates(self, symbol: str, interval: str = "1min") -> Dict:
        """获取外汇实时数据"""
        params = {
            'symbol': symbol,
            'interval': interval,
        ...
        try:
            ...
            if 'values' in data:
                return data
            elif 'code' in data:  # API错误代码
                return {"error": f"API Error {data.get('code')}: {data.get('message', 'Unknown error')}"}
            else:
                return {"error": "Unexpected response format"}
    ...
    def get_currency_pairs(self) -> Dict:
        ...
        try:
            response = self.session.get(f"{self.base_url}/forex_pairs", params=params, timeout=10)
            response.raise_for_status()
            data = response.json()
    ...
    def get_technical_indicators(self, symbol: str, indicator: str, interval: str = "1day") -> Dict:
        """获取技术指标"""
        params = {
            'symbol': symbol,
            'interval': interval,
        ...
        try:
            response = self.session.get(f"{self.base_url}/{indicator}", params=params, timeout=10)
            response.raise_for_status()
            data = response.json()
            
            if 'values' in data or indicator in ['rsi', 'macd', 'bollinger_bands']:
                return data
            elif 'code' in data:
                return {"error": f"API Error {data.get('code')}: {data.get('message', 'Unknown error')}"}
            else:
    ...
    def get_market_status(self) -> Dict:
        ...
        try:
            response = self.session.get(f"{self.base_url}/market_state", params=params, timeout=10)
            response.raise_for_status()
            data = response.json()
    ...
    def health_check(self) -> Dict:
        ...
        try:
            ...
            status = "operational" if 'values' in data else "degraded"
            remaining_requests = data.get('status', {}).get('remaining_requests', 'unknown')
            
            return {
                "status": status,
                "api_calls_left": remaining_requests,
                "response_time": response.elapsed.total_seconds()
            }
        except Exception as e:
            return {"status": "unavailable", "error": str(e)}
